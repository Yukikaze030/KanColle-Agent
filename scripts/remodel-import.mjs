...
export async function importRemodelData(root, outputDir, { refresh = false } = {}) {
  ...
  if (refresh || masterFile || cached?.source !== sources.master || cached?.schema_version !== 2) {
    ...
    for (const key of ["api_mst_ship", "api_mst_shipupgrade", "api_mst_useitem", "api_mst_slotitem", "api_mst_slotitem_equiptype", "api_mst_stype", "api_mst_mission", "api_mst_maparea", "api_mst_mapinfo"]) {
      if (!Array.isArray(master[key]) || !master[key].length) throw new Error(`Missing master table ${key}`);
    }
    const fields = ["api_id", "api_name", "api_yomi", "api_sortno", "api_stype", "api_ctype", "api_aftershipid", "api_afterlv", "api_afterfuel", "api_afterbull", "api_taik", "api_souk", "api_houg", "api_raig", "api_tyku", "api_luck", "api_kaih", "api_tais", "api_saku", "api_soku", "api_leng", "api_maxeq", "api_slot_num"];
    cached = {
      schema_version: 2,
      ...
      stypes: master.api_mst_stype,
      equip_types: master.api_mst_slotitem_equiptype,
      equip_ship: master.api_mst_equip_ship ?? {},
      equip_exslot: master.api_mst_equip_exslot ?? [],
      equip_exslot_ship: master.api_mst_equip_exslot_ship ?? {},
      equip_limit_exslot: master.api_mst_equip_limit_exslot ?? {},
      missions: master.api_mst_mission,
      mapareas: master.api_mst_maparea,
      maps: master.api_mst_mapinfo,
    };
  }
  // Run only the pinned, locally reviewed vendor snapshot; never fetched executable code.
  // Git may materialize the reviewed vendor snapshot with CRLF on Windows.
  // Hash and execute its canonical LF representation so checkout settings do not invalidate the pin.
